...
- `POST /rooms/:id/keys` — uploads one or more `RoomKeyGrant` rows (`{userId, keyId, wrappedKey}`), each sealed to the target member's current `publicKey`. Also the endpoint a rotation uses to atomically claim an epoch and grant it — see Rotation authority, below.
...
export interface ContentCipher {
  ...
  decrypt(content: string, ad: MessageAad): string;    // throws on AD/key mismatch
}
